...
void collect_job_stats(sqlite3 * jobs_db, char * out_dir, char * hostname, long time_sec){
	...
	char * cmd;
	asprintf(&cmd, "sacct --nodelist=%s --format=User,Group,JobID,ReqTRES,Timelimit,Submit,NodeList,Start,End,Elapsed,State,ExitCode --state=COMPLETED,CANCELLED,FAILED,TIMEOUT,OUT_OF_MEMORY --starttime=now-1day --endtime=now --unit=M --allusers -P", hostname);
